...
void read_test(Rhd2000EvalBoard& evalBoard) {
    int test_sample_rate = Rhd2000EvalBoard::SampleRate20000Hz;
    int test_time_sec[] = { 3, 5, 8 };
    int result[3];

    for (int i = 0; i < 3; i++) {

        result[i] = 20000 * test_time_sec[i] / 60;

        evalBoard.setSampleRate((Rhd2000EvalBoard::AmplifierSampleRate) test_sample_rate);
        evalBoard.setMaxTimeStep(20000 * test_time_sec[i]);
        ...
        evalBoard.flush();
        printf("%d %d\n", result[i], dataQueue.size());
        assert(result[i] == dataQueue.size());
    }
}
...
int main(int argc, char *argv[])
{
    ...
    do {
        usbDataRead = evalBoard->readDataBlocks(100, dataQueue);
            ...
            {
                cout << "total count : " << dataQueue.size() << endl;
                cout << "getNumEnabledDataStreams : " << evalBoard->getNumEnabledDataStreams() << endl;
                while (!dataQueue.empty()) {
                    dataQueue.front().write(cout, evalBoard->getNumEnabledDataStreams());
                    //dataQueue.pop();
                }
            }
