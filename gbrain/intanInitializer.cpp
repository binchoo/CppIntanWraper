...
bool IntanInitializer::checkValidityForRunningTime(string runningTime) {
	if (atoi(runningTime.c_str()) < 0)
		return false;
	return true;
}
	...
	while ((opt = getopt(argc, argv, "s:t:p:P:")) != -1) {
		switch (opt) {
			case 's':
				...
				break;
			case 't':
				if (checkValidityForRunningTime(optarg))
...
unsigned int IntanInitializer::getRuningTime() {
	return runningTime;
}
